...
public class AudioSetting : MonoBehaviour, IPointerUpHandler, IEndDragHandler
{
    ...
    private const float MinimumLogVolume = 0.0001f;
    private const float DecibelMultiplier = 20f;
    private const float ReleaseSoundCooldown = 0.05f;
    ...
    private void OnMuteToggled(bool isMuted)
    {
        ...
        {
            float saved = GetSavedVolume();
            float restoreVolume = Mathf.Max(saved, DefaultVolume);
    ...
    private void ApplyVolume(float value, bool isMuted)
    {
        ...
        float volumeDb = isMuted
            ? MutedVolumeDb
            : Mathf.Log10(Mathf.Max(clamped, MinimumLogVolume)) * DecibelMultiplier;
    ...
    private void PlayReleaseSound()
    {
        ...
        if (Time.unscaledTime - _lastPlayTime < ReleaseSoundCooldown)
        {
            return;
        }
